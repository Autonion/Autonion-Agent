...
class DesktopAgentService {
  ...
  Future<void> runTask(String goal, {AutomationTier tier = AutomationTier.accessibilityOnly, void Function(String)? onProgress}) async {
    ...
    try {
      while (steps < maxSteps && !_stopRequested) {
        ...
        final userPrompt = DesktopPromptFormatter.buildUserPrompt(goal, screenState, _history);
        
        final messages = [
          AiMessage(role: AiMessageRole.system, content: systemPrompt),
          // We could add history here, but keeping it simple for v1
          AiMessage(role: AiMessageRole.user, content: userPrompt, base64Image: screenState.screenshotBase64),
        ];
      ...
      if (_stopRequested) {
        ...
        _log.warn('DesktopAgent', 'Task reached max steps ($maxSteps) without completing.');
